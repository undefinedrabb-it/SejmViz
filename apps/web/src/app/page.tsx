...
import { clubsE as clubsE09, type ClubK as ClubK09, type ClubsEnhanedT as ClubsEnhanedT09, clubKs as clubKs09 } from "../data/clubs-09";
import { clubsE as clubsE10, type ClubK as ClubK10, type ClubsEnhanedT as ClubsEnhanedT10, clubKs as clubKs10 } from "../data/clubs-10";
import { mpsE as mpsE09, type MPEnhanedT as MPEnhanedT09 } from "../data/mps-09";
import { mpsE as mpsE10, type MPEnhanedT as MPEnhanedT10 } from "../data/mps-10";
...
const Home = () => {
  const [{ focused, mps, clubs, term }, dispatch] = useReducer(reducer, initialState);
  ...
  const setOrToggleIfSame = (clubId: ClubK) => () => {
    dispatch({
      type: "setFocused",
      payload: focused?.includes(clubId) ? focused.filter((ff) => ff !== clubId) : [...focused, clubId],
    });
  };
  return (
    <>
      ...
      <div className='flex flex-col h-screen'>
        <main className='grow bg-gray-800 flex-rows flex justify-around p-4'>
          ...
          <div className='flex flex-col gap-2 place-items-end'>
            <Listbox value={term} onChange={(v) => dispatch({ type: "setTerm", payload: v.value })}>
                  ...
                  <Listbox.Options className='absolute mt-1 max-h-60 min-w-100 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm'>
                    {terms.map((person, personIdx) => (
                      <Listbox.Option
                        key={personIdx}
                        className={({ active }) =>
                          `relative cursor-default select-none py-2 pl-10 pr-6 ${active ? "bg-amber-100 text-amber-900" : "text-gray-900"}`
                        }
                        value={person}
                      >
                        {({ selected }) => (
                          <>
                            ...
                            {selected ?
                              <span className='absolute inset-y-0 left-0 flex items-center pl-3 text-amber-600'>
                                a{/* <CheckIcon className="h-5 w-5" aria-hidden="true" /> */}
                              </span>
                            : null}
                          </>
                        )}
...
function Percentage({ focused, mps }: { focused: readonly ClubK[]; mps: { isHidden: boolean; id: number; club: ClubK; color: string }[] }) {
  const filterd = useMemo(() => mps.filter((mp) => focused.includes(mp.club)).length, [focused, mps]);
  const allMps = useMemo(() => mps.length, [mps]);

  return <div className='text-5xl text-white grow'>{filterd === allMps ? "All" : `${((filterd * 100) / allMps).toPrecision(2)}%`}</div>;
}
